/*  Downloaded from;
 *  ftp://ftp.dante.de/tex-archive/indexing/makeindexk/qsort.c
 *
 ...
 */
#include "sort_r.h"			/* qq_compar_fn_t type and qqsort() prototype */
...
static void qst(char *base, char *max, void *thun);
/*
...
void qqsort(void *base, size_t n, size_t size, qq_compar_fn_t compar, void *thun)
{
    register char *i;
    register char *j;
    register char *lo;
...
static void qst(char *base, char *max, void *thun)
{
	...
	if (lo >= mthresh) {
	    j = ((*qcmp) ((jj = base), i, thun) > 0 ? jj : i);
	    if ((*qcmp) (j, (tmp = max - qsz), thun) > 0) {
		/* switch to first loser */
		j = (j == jj ? i : jj);
		if ((*qcmp) (j, tmp, thun) < 0)
		    j = tmp;
	    }
	    if (j != i) {
		ii = qsz;
	...
	for (i = base, j = max - qsz;;) {
	    while (i < mid && (*qcmp) (i, mid, thun) <= 0)
		i += qsz;
	    while (j > mid) {
		if ((*qcmp) (mid, j, thun) <= 0) {
		    j -= qsz;
		    continue;
		}
